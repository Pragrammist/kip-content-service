...
public class FilmServiceGrpc : FilmServiceProto.FilmServiceProtoBase
{
    ...
    {
        await _interactor.DecrNotInterestingCount(request.FilmdId, context.CancellationToken);
    ...
    {   
        await _interactor.DecrWatchedCount(request.FilmdId, context.CancellationToken);
    ...
    {
        await _interactor.IncrNotInterestingCount(request.FilmdId, context.CancellationToken);

        return new IsSuccessResponse { Success = true};
    }
    public override async Task<IsSuccessResponse> IncrShareCount(FilmIdRequest request, Grpc.Core.ServerCallContext context)
    {
        await _interactor.IncrShareCount(request.FilmdId, context.CancellationToken);
    ...
    {
        await _interactor.IncrViewsCount(request.FilmdId, context.CancellationToken);
    ...
    {
        await _interactor.IncrWatchedCount(request.FilmdId, context.CancellationToken);
    ...
    {
        await _interactor.AddScore(request.FilmdId, request.Score, context.CancellationToken);
